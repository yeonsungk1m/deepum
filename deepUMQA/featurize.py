...
def _edge_iter_dereference(edge_iter):
    """
    Dereference a PyRosetta EdgeListConstIter in a version-tolerant way.
    PyRosetta historically exposed operator* as __mul__(), but newer builds
    provide pythonic helpers (e.g., value(), edge()). We probe a set of known
    attribute names to stay compatible across releases.
    """
    candidate_methods = (
        "__dereference__",
        "dereference",
    ...
    for name in candidate_methods:
        attr = getattr(edge_iter, name, None)
        if attr is None:
            continue
        try:
            return attr()
            ...
            try:
                # Some bound C++ methods require the iterator explicitly.
                return attr(edge_iter)
            except TypeError:
                continue

    # Some builds expose a property instead of a callable.
    for name in ("edge", "value"):
        attr = getattr(edge_iter, name, None)
        if attr is not None and not callable(attr):
            return attr

    # Fallback: try __getitem__ for implementations that expose array access.
    getitem = getattr(edge_iter, "__getitem__", None)
    if callable(getitem):
        try:
            return getitem(0)
        except Exception:
    ...
    raise AttributeError(
        "Unable to dereference PyRosetta EdgeListConstIter; the API may have changed."
    )


def _edge_iter_increment(edge_iter):
    """
    Advance a PyRosetta EdgeListConstIter safely.
    Returns the (possibly new) iterator instance after incrementing.
    """

    # 1) 최신 PyRosetta 스타일: pre_increment()
    pre_inc = getattr(edge_iter, "pre_increment", None)
    if callable(pre_inc):
        result = pre_inc()
        return edge_iter if result is None else result
    ...
    for name in candidates:
        ...
        if attr is None:
            continue
        for args in ((), (edge_iter,), (1,), (edge_iter, 1)):
            try:
                result = attr(*args)
            except TypeError:
                continue
    ...
    # 3) 일부 구현에서 next() 제공
    attr = getattr(edge_iter, "next", None)
    if callable(attr):
        result = attr()
        return edge_iter if result is None else result

    # 4) 파이썬 iterator 프로토콜 fallback
    if hasattr(edge_iter, "__iter__") and hasattr(edge_iter, "__next__"):
        try:
            edge_iter.__next__()
            return edge_iter
    ...
    available = ", ".join(sorted(name for name in dir(edge_iter) if not name.startswith("_")))
    raise AttributeError(
        "Unable to advance PyRosetta EdgeListConstIter; the API may have changed. "
        f"Iterator attributes: {available}"
    )


def _iter_from_container(container):
    """
    Attempt to turn a PyRosetta edge container into a Python iterator.
    Returns an iterator if successful, otherwise None.
    """
    ...
    # Some containers expose an explicit list/vector conversion.
    for name in ("to_list", "to_vector", "to_edge_list", "edges", "as_list"):
...
def _iter_energy_edges(node, length_hint):
    """
    Yield all edges for a node in the energy graph.
    Falls back to legacy iterator behaviour when the node is not directly iterable.
    """
    accessor_names = (
        "const_edge_list",
        "edge_list",
    ...
    for accessor_name in accessor_names:
        attr = getattr(node, accessor_name, None)
        if attr is None:
            continue
        ...
        if callable(attr):
            try:
                potential_containers.append(attr())
            except TypeError:
                try:
                    potential_containers.append(attr(node))
                except TypeError:
                    pass
    ...
    # --- 여기서부터 EdgeListConstIter 기반 fallback ---

    begin = node.const_edge_list_begin()

    # 안전 장치용 상한
    max_edges = max(4, length_hint) * max(4, length_hint) * 4
    counter = 0

    # 1) 최신 API: valid() / dereference() / pre_increment()
    if hasattr(begin, "valid") and hasattr(begin, "dereference"):
        while begin.valid():
            yield _edge_iter_dereference(begin)
            begin = _edge_iter_increment(begin)
            counter += 1
            if counter > max_edges:
                raise RuntimeError(
                    "Exceeded safety cap while iterating PyRosetta energy graph edges; "
                    "please check for API changes."
                )
        return

    # 2) 레거시: end iterator와 비교
    end = node.const_edge_list_end()
    while begin != end:
        yield _edge_iter_dereference(begin)
        begin = _edge_iter_increment(begin)
        counter += 1
...
def extract_EnergyDistM(pose, energy_terms):
    ...
    for i in range(length):
        ...
        node = graph.get_node(index1)

        # Parse the energy graph using a version-tolerant iterator.
        for edge in _iter_energy_edges(node, length):
...
def extract_USR(pose):
    hang = pose.size()
    # Get CA to CA distance map
    distance = get_distmaps(pose, atom1="CA", atom2="CA")
    avg1 = []
    avg2 = []
    avg3 = []
    index = 0
    for i in range(hang):
        avg1.append(np.average(distance[i]))  # 与当前残基q1的距离平均值avg1
        idx2 = np.argmax(distance, axis=1)  # 定位与当前残基q0的距离最远残基q2
        avg2.append(np.average(distance[idx2[i]]))  # 与残基q2的距离平均值avg2
        idx3 = np.argmax(distance[idx2[i]], axis=0)  # 定位与残基q1的距离最远残基q3
        avg3.append(np.average(distance[idx3]))  # 与残基q3的距离平均值avg3
